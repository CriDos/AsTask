...
namespace HardDev.AsTask.Coordination
{
    ...
    /// </summary>
    public static class QAsyncWaitQueueExtensions
    {
        /// <summary>
        ...
        public static Task<T> Enqueue<T>(this IAsyncWaitQueue<T> @this, object mutex, CancellationToken token)
        {
            ...
            ret.ContinueWith(_ => registration.Dispose(), CancellationToken.None, TaskContinuationOptions.ExecuteSynchronously,
    ...
    /// </summary>
    /// <typeparam name="T">The type of the results. If this isn't needed, use <see cref="object"/>.</typeparam>
    public sealed class QDefaultAsyncWaitQueue<T> : IAsyncWaitQueue<T>
    {
        private readonly QDeque<TaskCompletionSource<T>> _queue = new QDeque<TaskCompletionSource<T>>();
