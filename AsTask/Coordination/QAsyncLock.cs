...
namespace HardDev.AsTask.Coordination
{
    ...
    /// </summary>
    /// <remarks>
    /// <para>This is the <c>async</c>-ready almost-equivalent of the <c>lock</c> keyword or the <see cref="Mutex"/> type, similar to <a href="http://blogs.msdn.com/b/pfxteam/archive/2012/02/12/10266988.aspx">Stephen Toub's QAsyncLock</a>. It's only <i>almost</i> equivalent because the <c>lock</c> keyword permits reentrancy, which is not currently possible to do with an <c>async</c>-ready lock.</para>
    /// <para>An <see cref="QAsyncLock"/> is either taken or not. The lock can be asynchronously acquired by calling <see autoUpgrade="true" cref="LockAsync()"/>, and it is released by disposing the result of that task. <see cref="LockAsync(CancellationToken)"/> takes an optional <see cref="CancellationToken"/>, which can be used to cancel the acquiring of the lock.</para>
    /// <para>The task returned from <see autoUpgrade="true" cref="LockAsync()"/> will enter the <c>Completed</c> state when it has acquired the <see cref="QAsyncLock"/>. That same task will enter the <c>Canceled</c> state if the <see cref="CancellationToken"/> is signaled before the wait is satisfied; in that case, the <see cref="QAsyncLock"/> is not taken by that task.</para>
    /// <para>You can call <see cref="Lock(CancellationToken)"/> or <see cref="LockAsync(CancellationToken)"/> with an already-cancelled <see cref="CancellationToken"/> to attempt to acquire the <see cref="QAsyncLock"/> immediately without actually entering the wait queue.</para>
    /// </remarks>
    /// <example>
    ...
    /// <para>So, we use the <c>async</c>-compatible <see cref="QAsyncLock"/> instead:</para>
    /// <code>
    /// private readonly QAsyncLock _mutex = new QAsyncLock();
    ...
    /// </code>
    /// </example>
    public sealed class QAsyncLock
    {
        /// <summary>
        ...
        public QAsyncLock(IAsyncWaitQueue<IDisposable> queue)
        {
            _queue = queue ?? new QDefaultAsyncWaitQueue<IDisposable>();
            _mutex = new object();
        }
        ...
        /// </summary>
        public int Id => QIdManager<QAsyncLock>.GetId(ref _id);
        ...
        {
            return new QAwaitableDisposable<IDisposable>(RequestLockAsync(cancellationToken));
        }
        ...
        /// </summary>
        /// <returns>A disposable that releases the lock when disposed.</returns>
        public QAwaitableDisposable<IDisposable> LockAsync()
        {
        ...
        /// </summary>
        private sealed class Key : QSingleDisposable<QAsyncLock>
        {
            /// <inheritdoc />
            /// <summary>
            ...
            public Key(QAsyncLock asyncLock)
            {
                CreatesDisposableContext(asyncLock);
            }

            protected override void Dispose(QAsyncLock context)
            {
                context.ReleaseLock();
            }
        }
