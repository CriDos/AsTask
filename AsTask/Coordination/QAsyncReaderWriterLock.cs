...
namespace HardDev.AsTask.Coordination
{
    ...
    /// </summary>
    public sealed class QAsyncReaderWriterLock
    {
        /// <summary>
        ...
        public QAsyncReaderWriterLock(IAsyncWaitQueue<IDisposable> writerQueue, IAsyncWaitQueue<IDisposable> readerQueue)
        {
            _writerQueue = writerQueue ?? new QDefaultAsyncWaitQueue<IDisposable>();
            _readerQueue = readerQueue ?? new QDefaultAsyncWaitQueue<IDisposable>();
            _mutex = new object();
        }
        ...
        /// </summary>
        public int Id => QIdManager<QAsyncReaderWriterLock>.GetId(ref _id);
        ...
        private void ReleaseWaitersWhenCanceled(Task task)
        {
            task.ContinueWith(t =>
                ...
                }, CancellationToken.None, TaskContinuationOptions.OnlyOnCanceled | TaskContinuationOptions.ExecuteSynchronously,
        ...
        /// <param name="cancellationToken">The cancellation token used to cancel the lock. If this is already set, then this method will attempt to take the lock immediately (succeeding if the lock is currently available).</param>
        /// <returns>A disposable that releases the lock when disposed.</returns>
        public QAwaitableDisposable<IDisposable> ReaderLockAsync(CancellationToken cancellationToken)
        {
            return new QAwaitableDisposable<IDisposable>(RequestReaderLockAsync(cancellationToken));
        }
        ...
        /// </summary>
        /// <returns>A disposable that releases the lock when disposed.</returns>
        public QAwaitableDisposable<IDisposable> ReaderLockAsync()
        {
        ...
        /// <param name="cancellationToken">The cancellation token used to cancel the lock. If this is already set, then this method will attempt to take the lock immediately (succeeding if the lock is currently available).</param>
        /// <returns>A disposable that releases the lock when disposed.</returns>
        public QAwaitableDisposable<IDisposable> WriterLockAsync(CancellationToken cancellationToken)
        {
            return new QAwaitableDisposable<IDisposable>(RequestWriterLockAsync(cancellationToken));
        }
        ...
        /// </summary>
        /// <returns>A disposable that releases the lock when disposed.</returns>
        public QAwaitableDisposable<IDisposable> WriterLockAsync()
        {
        ...
        /// </summary>
        private sealed class ReaderKey : QSingleDisposable<QAsyncReaderWriterLock>
        {
            /// <summary>
            /// Creates the key for a lock.
            /// </summary>
            /// <param name="asyncReaderWriterLock">The lock to release. May not be <c>null</c>.</param>
            public ReaderKey(QAsyncReaderWriterLock asyncReaderWriterLock)
            {
                CreatesDisposableContext(asyncReaderWriterLock);
            }

            protected override void Dispose(QAsyncReaderWriterLock context)
            {
        ...
        /// </summary>
        private sealed class WriterKey : QSingleDisposable<QAsyncReaderWriterLock>
        {
            /// <summary>
            /// Creates the key for a lock.
            /// </summary>
            /// <param name="asyncReaderWriterLock">The lock to release. May not be <c>null</c>.</param>
            public WriterKey(QAsyncReaderWriterLock asyncReaderWriterLock)
            {
                CreatesDisposableContext(asyncReaderWriterLock);
            }
