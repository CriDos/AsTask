...
namespace HardDev.AsTask.TaskHelpers.Interop
{
    /// <summary>
    /// Creation methods for tasks wrapping events.
    /// </summary>
    public static class QEventAsyncFactory
    {
        /// <summary>
            ...
            Action<TDelegate> subscribe, Action<TDelegate> unsubscribe, CancellationToken cancellationToken, bool unsubscribeOnCapturedContext)
        {
            ...
            var tcs = QTaskCompletionSourceExtensions.CreateAsyncTaskSource<TEventArguments>();
            ...
            try
            {
                using (cancellationToken.Register(() => tcs.TrySetCanceled(), false))
                {
                    ...
                    return await tcs.Task.ConfigureAwait(unsubscribeOnCapturedContext);
                }
            }
            finally
            {
        ...
        /// <para>Calling this method in a loop is often an anti-pattern, because the event is only subscribed to when this method is invoked, and is unsubscribed from when the task completes. From the time the task is completed until this method is called again, the event may fire and be "lost". If you find yourself needing a loop around this method, consider using Rx or TPL Dataflow instead.</para>
        /// </remarks>
        public static Task<QEventArguments<object, EventArgs>> FromEvent(Action<EventHandler> subscribe, Action<EventHandler> unsubscribe,
            CancellationToken cancellationToken, bool unsubscribeOnCapturedContext)
            => FromAnyEvent<EventHandler, QEventArguments<object, EventArgs>>(x => (sender, args) => x(CreateEventArguments(sender, args)), subscribe,
                unsubscribe, cancellationToken, unsubscribeOnCapturedContext);
        ...
        /// <para>Calling this method in a loop is often an anti-pattern, because the event is only subscribed to when this method is invoked, and is unsubscribed from when the task completes. From the time the task is completed until this method is called again, the event may fire and be "lost". If you find yourself needing a loop around this method, consider using Rx or TPL Dataflow instead.</para>
        /// </remarks>
        public static Task<QEventArguments<object, TEventArgs>> FromEvent<TEventArgs>(Action<EventHandler<TEventArgs>> subscribe,
            Action<EventHandler<TEventArgs>> unsubscribe, CancellationToken cancellationToken, bool unsubscribeOnCapturedContext)
            => FromAnyEvent<EventHandler<TEventArgs>, QEventArguments<object, TEventArgs>>(
        ...
        /// </remarks>
        public static Task<QEventArguments<object, TEventArgs>> FromEvent<TEventArgs>(Action<EventHandler<TEventArgs>> subscribe,
            Action<EventHandler<TEventArgs>> unsubscribe, CancellationToken cancellationToken)
            => FromEvent(subscribe, unsubscribe, cancellationToken, true);
        ...
        /// <para>Calling this method in a loop is often an anti-pattern, because the event is only subscribed to when this method is invoked, and is unsubscribed from when the task completes. From the time the task is completed until this method is called again, the event may fire and be "lost". If you find yourself needing a loop around this method, consider using Rx or TPL Dataflow instead.</para>
        /// </remarks>
        public static Task<QEventArguments<object, TEventArgs>> FromEvent<TDelegate, TEventArgs>(Func<EventHandler<TEventArgs>, TDelegate> convert,
            Action<TDelegate> subscribe, Action<TDelegate> unsubscribe, CancellationToken cancellationToken, bool unsubscribeOnCapturedContext)
            => FromAnyEvent<TDelegate, QEventArguments<object, TEventArgs>>(x => convert((sender, args) => x(CreateEventArguments(sender, args))),
                subscribe, unsubscribe, cancellationToken, unsubscribeOnCapturedContext);
        ...
        /// <para>Calling this method in a loop is often an anti-pattern, because the event is only subscribed to when this method is invoked, and is unsubscribed from when the task completes. From the time the task is completed until this method is called again, the event may fire and be "lost". If you find yourself needing a loop around this method, consider using Rx or TPL Dataflow instead.</para>
        /// </remarks>
        public static Task<QEventArguments<TSender, TEventArgs>> FromActionEvent<TSender, TEventArgs>(Action<Action<TSender, TEventArgs>> subscribe,
            Action<Action<TSender, TEventArgs>> unsubscribe, CancellationToken cancellationToken, bool unsubscribeOnCapturedContext)
            => FromAnyEvent<Action<TSender, TEventArgs>, QEventArguments<TSender, TEventArgs>>(
                x => (sender, args) => x(CreateEventArguments(sender, args)), subscribe, unsubscribe, cancellationToken,
                unsubscribeOnCapturedContext);
        ...
        /// </remarks>
        public static Task<QEventArguments<TSender, TEventArgs>> FromActionEvent<TSender, TEventArgs>(Action<Action<TSender, TEventArgs>> subscribe,
            Action<Action<TSender, TEventArgs>> unsubscribe, CancellationToken cancellationToken)
        ...
        /// <summary>
        /// Creates an <see cref="QEventArguments{TSender,TEventArgs}"/> structure.
        /// </summary>
        ...
        private static QEventArguments<TSender, TEventArgs> CreateEventArguments<TSender, TEventArgs>(TSender sender, TEventArgs eventArgs)
            => new QEventArguments<TSender, TEventArgs> {Sender = sender, EventArgs = eventArgs};
    }
}