...
public class Tests1
{
    ...
    [Test]
    public async Task TestThreadPool1()
    {
        var tp = new StaticThreadPool("main", 1);

        await tp.Awaiter;

        Assert.That(AsTask.WhereAmI(), Is.EqualTo("CustomThreadPool(name=main; type=StaticThreadPool)"));

        _ = tp.TaskFactory.StartNew(() => { Thread.Sleep(500); });
        _ = tp.TaskFactory.StartNew(() => { Thread.Sleep(1000); });

        await AsTask.ToMainContext();

        Assert.True(tp.ExecutableTasks == 0);
    }
}